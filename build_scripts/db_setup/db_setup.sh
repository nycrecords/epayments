...
openssl req \
       ...
       -x509 -days 365 -out /vagrant/build_scripts/db_setup/server.crt -subj "/C=US/ST=New York/L=New York/O=NYC Department of Records and Information Services/OU=IT/CN=epayments.dev"
...
if [ "$1" = single_server ]; then
  ...
  openssl req -new -nodes -keyout client.key -out client.csr -subj "/C=US/ST=New York/L=New York/O=NYC Department of Records and Information Services/OU=IT/CN=epayments.dev"
...
sudo -u postgres /opt/rh/rh-postgresql95/root/usr/bin/createuser -s -e epayments_db

# 12. Create database
sudo -u postgres /opt/rh/rh-postgresql95/root/usr/bin/createdb epayments
