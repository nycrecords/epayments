...
def get_orders():
    ...
    if request.form:
        ...
        orders = [order.serialize for order in Orders.query.filter_by(datereceived=yesterday).all()]
        return jsonify(orders=orders)
...
def get_orders_by_fields(order_number, suborder_number, order_type, billing_name, user, date_received,
    ...
    orders = Orders.query.filter(Orders.datereceived >= date_received, Orders.datereceived <= date_submitted)
    ...
    orders = Orders.query.filter(Orders.datereceived >= date_received_start, Orders.datereceived <= date_received_end)
    if len(order_number) != 0:
        orders = orders.filter(Orders.clientid == order_number)
    if len(suborder_number) != 0:
        orders = orders.filter(Orders.suborderno == suborder_number)
    if len(billing_name) != 0:
        orders = orders.filter(func.lower(Orders.billingname).contains(func.lower(billing_name)))
    if len(order_type) != 4 and order_type not in ['All', 'multipleitems', 'vitalrecordsphotos']:
        orders = orders.filter(Orders.clientagencyname == order_type)
...
def get_orders_by_fields_dict(order_number, suborder_number, order_type, billing_name, user, date_received,
    ...
    orders = [
        {
            ...
            "datereceived": "2017-06-13 00:00:00",
            "datesubmitted": "Tue, 13 Jun 2017 00:00:00 GMT",
            "orderno": "000592000",
    ...
    if len(date_received) < 1:
        ...
        date_received = datetime.datetime.strptime(date_received, "%m/%d/%Y").date()
    if len(date_submitted) < 1:
        date_submitted = yesterday
    else:
        date_submitted = datetime.datetime.strptime(date_submitted, "%m/%d/%Y").date()
